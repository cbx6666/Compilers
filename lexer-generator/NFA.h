...
public:
    int id;
    map<char, vector<shared_ptr<NFAState>>> transitions;  // 转换表：字符 -> 目标状态列表
...
public:
    ...
    NFA(shared_ptr<NFAState> s, shared_ptr<NFAState> a) 
        : start(s), accept(a) {
        accept->isAccepting = true;
    }
    
    // 计算epsilon闭包
    set<shared_ptr<NFAState>> epsilonClosure(const set<shared_ptr<NFAState>>& states);
