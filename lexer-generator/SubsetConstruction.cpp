...
shared_ptr<DFA> SubsetConstruction::convert(shared_ptr<NFA> nfa) {
    ...
    set<shared_ptr<NFAState>> initialClosure = nfa->epsilonClosure(initialNfaStates);
    ...
    while (!unprocessed.empty()) {
        ...
        for (char c : alphabet) {
            // 计算move操作
            set<shared_ptr<NFAState>> moveResult = move(currentDfaState->nfaStates, c);
            
            if (!moveResult.empty()) {
                // 计算epsilon闭包
                set<shared_ptr<NFAState>> closure = nfa->epsilonClosure(moveResult);
...
set<shared_ptr<NFAState>> SubsetConstruction::move(const set<shared_ptr<NFAState>>& states, char c) {
