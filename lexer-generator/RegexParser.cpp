...
bool RegexParser::isEnd() { return pos >= input.length(); }
...
shared_ptr<ASTNode> RegexParser::parseAtom() {
  ...
  if (match('\\')) {
    ...
    switch (escaped) {
    case 'n':
      return make_shared<CharNode>('\n');
    case 't':
      return make_shared<CharNode>('\t');
    case 'r':
      return make_shared<CharNode>('\r');
    default:
      return make_shared<CharNode>(escaped);
    }
  }

  // 处理任意字符 .（匹配除换行符外的所有字符）
  if (match('.')) {
    return make_shared<AnyNode>();
  }
